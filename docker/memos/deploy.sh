#!/usr/bin/env bash
set -e
CREATE_DIRS=( "${BAK_CFG_DIR}"/memos/data )
mkdir -p "${CREATE_DIRS[@]}"
# ls -alh "$BAK_CFG_DIR"/memos/ "$DB_CFG_DIR"
ls -alh "${CREATE_DIRS[@]}"
docker compose  --env-file "$BAK_CFG_DIR/memos/.env" -f "${COMPOSE_DIR}/memos/docker-compose.yml" up -d --force-recreate
